...
class Adapter:
    def get_yamlt_comparator_results(ground_truth_ecore_model, predicted_ecore_model, projectName, config):
        yamtl_comparator_endpoint = f'{yamtl_comparator_url}/compare'
        response = requests.post(
            yamtl_comparator_endpoint,
            files={
                "groundTruthModel": ground_truth_ecore_model,
                "predictedModel": predicted_ecore_model,
                "config": config
            },
            data={
